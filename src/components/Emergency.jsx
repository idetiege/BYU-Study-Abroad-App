...
export default function Emergency() {
  return (
    <div className="overflow-y-auto" style={{ background: '#FFFFFF', paddingBottom: 'calc(4rem + env(safe-area-inset-bottom))' }}>
      {/* Header */}
      <div className="px-4 pb-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '12px', margin: 0 }}>Tap any number to call</p>
          </div>
        </div>
      </div>
      ...
      {sections.map(section => {
        ...
        return (
          <div key={section.title} className="px-4 mt-5">
            <p style={{ color: '#A3876F', fontSize: '11px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '10px' }}>
              {section.title}
            </p>
            <div className="flex flex-col gap-2">
              {contacts.map(contact => (
                <div key={contact.label}
                     ...
                     style={{ background: '#F5F0E8', border: '1px solid rgba(7,60,119,0.08)' }}>
                  <div className="flex-1 mr-3">
                    <p style={{ color: '#073C77', fontSize: '15px', fontWeight: 600, margin: '0 0 2px' }}>{contact.label}</p>
                    <p style={{ color: '#A3876F', fontSize: '12px', margin: '0 0 4px' }}>{contact.notes}</p>
