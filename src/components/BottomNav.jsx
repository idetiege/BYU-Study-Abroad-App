...
function HomeIcon({ active }) {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill={active ? '#C9A84C' : 'none'} stroke={active ? '#C9A84C' : '#8A8A9A'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function CalendarIcon({ active }) {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={active ? '#C9A84C' : '#8A8A9A'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] z-50"
         style={{ background: 'rgba(10,25,49,0.97)', backdropFilter: 'blur(16px)', borderTop: '1px solid rgba(201,168,76,0.15)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="flex">
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span style={{ fontSize: '10px', fontWeight: 600, color: isActive ? '#C9A84C' : '#8A8A9A', letterSpacing: '0.5px', textTransform: 'uppercase' }}>
                  {label}
                </span>
              </>
            )}
